...
module.exports = {
      ...
      _.each(region.rounds, function(round, roundIndex) {
        ...
        _.each(round.games, function(game, gameIndex) {
          ...
          if (!editable && validatedMaster && roundIndex > 0 && game !== this.unpickedGame) {
            var masterGame = validatedMaster.regions[regionIndex].rounds[roundIndex].games[gameIndex];
  ...
  score: function(user, master, name) {
    ...
    _.each(validatedUser.regions, function(region, region_i) {
      var firstRoundScore = (region.id === this.finalFourRegionName) ? 160 : 10;
      _.each(region.rounds, function(round, round_i) {
        if (round_i > 0) {
          _.each(round.games, function(game, game_i) {
            var masterGame = validatedMaster.regions[region_i].rounds[round_i].games[game_i],
                gameScore = firstRoundScore * round_i,
                scoreIndex = (region.id === this.finalFourRegionName) ? roundScores.length : round_i - 1;
                
            if (typeof roundScores[scoreIndex] === 'undefined') roundScores[scoreIndex] = 0;
                
            roundScores[scoreIndex] += (masterGame !== this.unpickedGame && game === masterGame) ? gameScore : 0;
          }, this);
        }
      }, this);
    }, this);
