  ...
  score: function(user, master) {
    ...
    _.each(validatedUser.regions, function(region, region_i) {
      _.each(region.rounds, function(round, round_i) {
        if (round_i === 0 && region.id !== 'FF') {
          
        } else {
          _.each(round.games, function(game, game_i) {
            score += (game !== this.unpickedGame && game === validatedMaster.regions[region_i].rounds[round_i].games[game_i]) ? firstRoundScore * round_i : 0;
          }, this);
        }
      }, this);
    }, this);
