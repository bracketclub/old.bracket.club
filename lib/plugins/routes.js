...
function renderHome() {
  this.res.writeHead(200, {"Content-Type": "text/html"});
  this.res.end(app.render('index', '', { bracket: renderBracket((editable) ? getEmptyCode() : getCurrentCode()), editable: editable }));
}
  ...
  if (process.env.DATABASE_URL) {
    ...
    pg.connect(process.env.DATABASE_URL, function(err, client) {
      if (err) {
        ...
        client.query('SELECT bracket, username FROM brackets', function(err, result) {
...
function renderUser(username) {
  ...
  if (process.env.DATABASE_URL) {
      ...
      if (err) {
        ...
        client.query('SELECT * FROM brackets WHERE username = $1 ORDER BY created DESC LIMIT 1', [username.toLowerCase()], function(err, result) {
          if (err || result.rows.length < 1) {
            ...
            _this.res.writeHead(200, {"Content-Type": "text/html"});
            _this.res.end(app.render('user', '', _.extend({bracket: renderBracket(result.rows[0].bracket), editable: editable}, result.rows[0])));
          }
        });
      }
    });
    ...
    this.res.writeHead(200, {"Content-Type": "text/html"});
    this.res.end(app.render('user', '', {bracket: renderBracket('S18541137214112424W185463721532533E191213113102112111011111MW1854113728432828FFWMWW'), username: username, editable: editable}));
  }
