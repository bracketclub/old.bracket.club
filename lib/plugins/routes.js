...
function renderUser(userName) {
  ...
  pg.connect(process.env.DATABASE_URL, function(err, client) {
    var query = client.query('SELECT bracket FROM brackets WHERE username = $1 LIMIT 1', [userNameLc]);
    query.on('row', function(result) {
      _this.res.writeHead(200, {"Content-Type": "text/html"});
      _this.res.end(app.render('user', '', {
        bracket: renderBracket(result.bracket),
        username: userName
      }));
    });
  });
