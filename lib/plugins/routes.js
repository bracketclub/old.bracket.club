...
function renderBracket(bracketCode) {
  return app.render('bracket', 'layouts/empty', validator.validateTournament(bracketCode, editable), bracketCode);
}
...
function renderUser(username) {
  ...
  pg.connect(process.env.DATABASE_URL, function(err, client) {
    if (err) {
      ...
      client.query('SELECT bracket FROM brackets WHERE username = $1 ORDER BY created ASC LIMIT 1', [username.toLowerCase()], function(err, result) {
        console.log('select user err', err);
        console.log('select user results', result);
