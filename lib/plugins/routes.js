var path = require('path'),
    ...
    editable = true,
    pg = require('pg'),
    databaseCon = process.env.DATABASE_URL || '';
...
function renderUser(userName) {
  var _this = this;
  pg.connect(databaseCon, function(err, client) {
    if (err) {
      console.log(err);
      _this.res.writeHead(404, {"Content-Type": "text/html"});
      _this.res.end(app.render('404'));
    } else {
      var query = client.query('SELECT bracket FROM brackets WHERE username = $1', [userName]);
      query.on('row', function(result) {
        if (!result) {
          ...
          _this.res.end(app.render('user', '', { bracket: renderBracket(result.bracket) }));
        }
      });
    }
  });
