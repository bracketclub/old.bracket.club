var path = require('path'),
    ...
    pg = require('pg'),
    correctBracket = 'S1812411310151431511515W1954637151437131E185463721432121MW181213113102112321222FFSEE';
...
function getCurrentCode() {
  if (process.env.DATABASE_URL) {
    return 'S18124113101514310131W1954637151437474E185463721462122MW181213113102113112122FFSMWS';
  } else {
    return correctBracket;
  }
}

function renderBracket(bracketCode, forceEditable) {
  return app.render('bracket', 'layouts/empty', validator.validateTournament(bracketCode, forceEditable || editable(), false, getCurrentCode()));
}

function renderCode(bracketCode) {
  this.res.writeHead(200, {"Content-Type": "text/html"});
  if (!/[0-9X]/.test(bracketCode)) bracketCode = getEmptyCode();
  this.res.end(renderBracket(bracketCode));
}

function renderEditable() {
  this.res.writeHead(200, {"Content-Type": "text/html"});
  this.res.end(app.render('index', '', { bracketDisplay: renderBracket(getEmptyCode(), true), editable: true }));
}
...
function renderResults() {
  ...
  if (process.env.DATABASE_URL) {
    ...
    var scores = [
    {username: 'test', bracket: correctBracket},
    {username: 'test2', bracket: 'S1812411310151431511515W1954637151437133E185463721432121MW181213113102112321222FFSEE'},
...
function renderUser(username) {
  ...
  if (process.env.DATABASE_URL) {
    ...
    this.res.writeHead(200, {"Content-Type": "text/html"});
    this.res.end(app.render('user', '', {bracketDisplay: renderBracket(correctBracket), username: username, editable: editable()}));
  }
...
exports.attach = function(options) {
  ...
  app.router.get('/editable', renderEditable);
