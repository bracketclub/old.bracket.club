...
function renderHome() {
  this.res.writeHead(200, {"Content-Type": "text/html"});
  this.res.end(app.render('index', '', { bracket: renderBracket(getEmptyCode()), editable: editable }));
}

function renderResults() {
  this.res.writeHead(200, {"Content-Type": "text/html"});
  this.res.end(app.render('results'), '', { editable: editable });
}
      ...
      if (err) {
        ...
        client.query('SELECT bracket FROM brackets WHERE username = $1 ORDER BY created DESC LIMIT 1', [username.toLowerCase()], function(err, result) {
          if (err || result.rows.length < 1) {
            ...
            _this.res.end(app.render('user', '', {bracket: renderBracket(result.rows[0].bracket), username: username, editable: editable}));
          }
        });
      }
    });
