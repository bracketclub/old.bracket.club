...
query = client.query('CREATE TABLE brackets (username varchar(50) NOT NULL, bracket varchar(140) NOT NULL, score integer DEFAULT 0, userid varchar(25) NOT NULL, tweetid varchar(25) PRIMARY KEY)');
...
twit.stream('statuses/filter', {track: '#tybrkt'}, function(stream) {
  stream.on('data', function(data) {
    ...
    if (validHash && Date.compare(new Date(Date.parse(data.created_at)), stopTime) === -1) {
