...
function renderResults(hasGooley) {
      ...
      db.findAll(function(err, entries) {
        ...
        async.concat(entries, function(entry, cb) {
          ...
          var rankCounter = 1,
              totalRanks = 1,
              lastUserScore = null,
              rankedScores = _.sortBy(results, 'totalScore').reverse(),
              rankedScore,
              tiedRanks = 0;
          ...
          for (var i = 0, m = rankedScores.length; i < m; i++) {
            ...
            if (lastUserScore === null) {
              rankCounter = 1;
            } else if (rankedScore.totalScore === lastUserScore) {
              tiedRanks++;
            } else {
              tiedRanks++;
              rankCounter += ((tiedRanks > 0) ? tiedRanks : 1);
              tiedRanks = 0;
            }
...
var routes = {
  ...
  index: function(req, res) {
    var renderIndex = function(err, bracketString) {
      new BracketValidator({flatBracket: bracketString}).validate(function(err, bracketData) {
        if (err) return render404(req, res);
        res.render('index', getBracketData(bracketData, {bracket: bracketString, title: 'Create Your Bracket!', live: isOpen()}));
      });
    };
  ...
  results: renderResults(false),
  gooley: renderResults(true),
