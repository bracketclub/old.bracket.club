...
export const push = (path, state) =>
  historyDispatcher('push', queryToSearch(path), state)
export const replace = (path, state) =>
  historyDispatcher('replace', queryToSearch(path), state)
...
export const replaceQuery = ({ location, query }) =>
  replace({ ...location, query: { ...(location.query || {}), ...query } })
