...
export default class Pick extends Component {
  ...
  render() {
    ...
    if (bestOf) {
      ...
      const winsInDisplay = (val) =>
        Array.isArray(bestOf.bestOf) ? bestOf.bestOf[val - 1] : val
      ...
      return (
        ...
        >
          <Team
            bsRole="toggle"
            className={cx(styles.team, styles.pickable)}
            {...team}
          >
            ...
            {bestOf.range.map((winsIn) => (
              <MenuItem
                ...
                componentClass={(props) => (
                  <SafeAnchor {...props} className={styles.seriesItemInner} />
                )}
              >
