...
const isTeamEqual = (team1, team2) =>
  isEqual(pick(team1, 'seed', 'name'), pick(team2, 'seed', 'name'))
...
export default class Bracket extends Component {
  ...
  renderTeam(team, { opponent, winner, top } = {}) {
    ...
    if (onUpdate) {
      return (
        <Pick
          {...{ onUpdate, bestOf, team, opponent, winner, isWinner, top }}
        />
      )
    }
    ...
    if (diff) {
      return <DiffPick {...{ team, bestOf, picked: winner }} />
    }
    ...
    return (
      <Team {...team}>
        {bestOf && get(winner, 'winsIn') && (
          <span className={styles.winsIn}>{winsInDisplay(winner)}</span>
        )}
      </Team>
    )
  }
  ...
  render() {
    ...
    if (bracket instanceof Error) {
      return (
        <Alert bsStyle="danger">
          ...
          <p>
            Could be that{' '}
            <strong>{bracket.message.toLowerCase().replace('.', '')}</strong>?
          </p>
        </Alert>
      )
    }
    ...
    const regionOpponent = (side, index) => {
      ...
      const opponentRegion = singleRegion
        ? first(opposite)
        : side[index === 0 ? 1 : 0]
