...
const friendsEntries = createSelector(
  ...
  ($entries, $me, $canWin) => {
    ...
    return $entries
      .filter(
        ($entry) =>
          (friends || []).indexOf($entry.user.id) > -1 || $entry.user.id === id
      )
      .map(($entry) => ({
        ...$entry,
        canWin: $canWin[$entry.id],
...
export const sortParams = createSelector(urlSort, ($urlSort) =>
  typeof $urlSort === 'undefined'
...
const orderEntries = ($entries, $score, $master, $sort) => {
  ...
  return orderBy(
    ...
    [$sort.dir, 'desc', 'desc', 'desc', 'desc']
  )
}

const entriesScoredByEvent = (entriesSelector) =>
  createSelector(
    entriesSelector,
