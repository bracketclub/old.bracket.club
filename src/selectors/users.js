...
const userEventId = createSelector(userId, eventSelectors.id, (...args) =>
  args.join('/')
)
...
const mapUserToEntries = ($user, $entries) => {
  ...
  const grouped = groupBy(
    orderBy($userEntries, 'created', 'desc'),
    ({ sport, year }) => sport + year
  )
  return map(grouped, (value) => value[0])
}

const visibleUser = (userSelector) =>
  createSelector(
    meId,
...
export const userWithScoredEntry = createSelector(
  ...
  ($user, $score, $master) => ({
    ...
    score:
      $user &&
      $master &&
      $score({ master: $master, entry: $user.entry.bracket }),
  })
)
...
export const userWithRankedEntry = createSelector(
  ...
  ($user, $entries) => ({
    ...$user,
    ...pick(
      $entries.find((entry) => entry.user.id === $user.id),
      'score'
    ),
  })
)
