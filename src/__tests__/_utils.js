...
export const configureStore = (initialState = {}) => {
  const store = createStore(
    rootReducer,
    initialState,
    compose(applyMiddleware(thunk, apiMiddleware, apiRelationsMiddleware))
  )
...
export const mockApi = (url, resp, { code = 200, delay = 100 } = {}) =>
  nock(config.apiUrl).get(url).delay(delay).reply(code, resp)

export const invokeSeries = (actions) => Promise.mapSeries(actions, (a) => a())
