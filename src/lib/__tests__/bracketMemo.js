...
const bracketsByYear = (bracketYears) =>
  bracketYears.map((year) => bracket({ sport, year }))
...
it('Initial teams are correct', () => {
  ...
  const teams = _.chain(brackets)
    .map('bracket')
    .map((b) => b.regions[_.keys(b.regions)[0]].teams)
    .map('0')
    .value()

  const validated = _.chain(brackets)
    .map((b) => {
  ...
  expect(validated.join(' ')).toBe(
    'S1Kentucky MW1Louisville S1Florida MW1Kentucky'
  )
...
it('Initial teams are correct', () => {
  ...
  const teams = _.chain(bracketsByYear(['2012', '2013', '2014']))
    .map(winner)
    .value()
  const teams2 = _.chain(bracketsByYear(['2014', '2013', '2012']))
    .map(winnerBackward)
    .value()
